...
async function postDataQueries (postData: PostDataObject){
  ...
  try {
    ...
    let collection: any = await CollectionDbServices.customGetCollection('cluster_id = $1 AND name = $2', [apiKey, collection_name]);

    if(!collection?.collection_id){
      // create new collection with cluster_id and name
      newCollection = await CollectionDbServices.createCollection({ cluster_id: apiKey, name: collection_name })
    ...
    let newDocument = await DocumentDbServices.createDocument({cluster_id: apiKey, collection_id, collection_name, data })
...
async function readAllDataQueries(readAllData: ReadDataObject ) {
...
async function readOneDataQueries(readOneData: ReadOneDataObject ) {
...
async function updateOneDataQueries(updateOneData: UpdateOneDataObject ) {
...
async function deleteOneDataQueries(deleteOneData: DeleteOneDataObject) {
